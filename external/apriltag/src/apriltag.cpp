...
namespace basalt {
    ...
    struct ApriltagDetectorData {
        ApriltagDetectorData(int numTags, AprilTags::TagCodes tagCodes)
    ...
    ApriltagDetector::ApriltagDetector(int numTags, std::string tagFamily, int startId) : _startId(startId), _tagFamily(tagFamily) {

        // std::cout << "ApriltagDetector uses tag family " << tagFamily << std::endl;

        if (tagFamily == "16h5") {
            data = new ApriltagDetectorData(numTags, AprilTags::tagCodes16h5);
        }else if (tagFamily == "36h11") {
            data = new ApriltagDetectorData(numTags, AprilTags::tagCodes36h11);
        }else
            throw std::runtime_error("tagFamily not supported: " + tagFamily);
   ...
   */
        ...
        for (iter = detections.begin(); iter != detections.end();) {
            ...
            // give more strict restrictions on the hamming distance
            if (_tagFamily == "16h5")
                remove |= iter->hammingDistance > 0;
            else if (_tagFamily == "36h11")
                remove |= iter->hammingDistance > 1;
            else
                throw std::runtime_error("unknown tag family for hamming distance threshold");
