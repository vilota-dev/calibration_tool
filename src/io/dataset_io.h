...
namespace basalt {
  ...
  typedef std::shared_ptr<RosbagVioDataset> VioDatasetPtr;
  ...
  public:
    ...
    void read(const std::string &path) {
      ...
      auto &cam_topics = data->cam_topics;
      auto &imu_topic = data->imu_topic;
      ...
      spdlog::info("imu_topic: {}", imu_topic);
      spdlog::info("mocap_topic: {}", mocap_topic);
      std::string topics_str;
      for (const std::string &s: cam_topics)
        topics_str += s + " ";
      spdlog::info("cam_topics: {}", topics_str);
  ...
  using DatasetIoInterfacePtr = std::shared_ptr<RosbagIO>;

  class DatasetIoFactory {
  public:
    static DatasetIoInterfacePtr getDatasetIo(
