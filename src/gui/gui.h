...
//  std::map<std::string, std::vector<std::string>> topics_to_message_types;
  ...
  void feed_images() {
    for (int64_t t_ns: this->image_timestamps) {
      ...
      const std::vector<basalt::ImageData> &img_vec = this->rosbag_io.get_data()->get_image_data(t_ns);

      for (size_t cam_id = 0; cam_id < this->rosbag_io.get_data()->get_num_cams(); cam_id++) {
        const basalt::ManagedImage<uint16_t>::Ptr &managed_image_ptr = img_vec[cam_id].img;
...
void run_gui() {
  ...
  Dataset big_data;
//  big_data.loadDataset(
//          "/Users/tejas/Developer/vilota-dev/calibration_tool/data/2023-04-21/run1/2023-04-21-vk180-calib-run1.bag");
...
//  }
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    if (ImGui::Begin("Image Display")) {
      ImmVision::ImageDisplay("Cat", app_state.image_data.at(app_state.image_timestamps[0])[0]);
      ImmVision::ImageDisplay("Dog", app_state.image_data.at(app_state.image_timestamps[0])[1]);
      ImmVision::ImageDisplay("Pig", app_state.image_data.at(app_state.image_timestamps[0])[2]);
    }
    ImGui::End();
